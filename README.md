...
- Manifest-enabled deltas support `auto`, `fast`, and `deep` reconciliation. The default `auto` mode uses fast reconciliation while a compatible deep baseline is less than 24 hours old and automatically runs deep when a baseline is missing, migrated state lacks the required signatures, the maximum age expires, or `forceDeepReconciliation=true`. `fast` performs a lightweight O(N) scan of every current root for presence, eligibility, and document ownership, but resolves complete payloads only for dirty roots; `deep` regenerates and checksums every current root while still uploading only changed operations.
